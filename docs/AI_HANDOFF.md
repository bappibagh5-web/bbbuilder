...
Current update (2026-09-13): M3-01 outreach domain foundation is implemented, awaiting manual validation; M3-02 is not started. The new `apps.outreach` models/services and migration preserve exact Ready scope-version, approved-candidate, contact, recipient status-event, and immutable prepared-message history. There is no sending, provider, production Outreach UI or API. Check Git and `docs/CURRENT_STATUS.md` for the authoritative current state; the older checkpoint below is historical.
