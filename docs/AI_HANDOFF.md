...
Current update (2026-09-17): M3-01 through M3-07 are COMPLETE and manually validated; M3-08 is NEXT / NOT STARTED. M3-07 preserves immutable quote intake and adds human-controlled structured BidRevisions plus explicit, suggestion-only native-text PDF extraction. Schema V3 live validation passed with `gpt-5-mini`: allowance inclusion is structured, equipment/procurement lead time remains a condition, validity and work schedule populate dedicated summary fields, grounded Base Bid currency is confirmed, and unlinked scope coverage is not guessed. Historical Run 1 succeeded under Schema V1; Runs 2 and 3 remain preserved `provider_unavailable` history caused by restricted local Celery outbound socket access, not OpenAI authentication/model access. Production Celery must run in a normal process context with outbound HTTPS access. Check Git and `docs/CURRENT_STATUS.md` for authoritative state; older checkpoints below are historical.

M3-07 commercial truth is human-reviewed BidRevision data layered over immutable quote files, never AI output. A structured revision is created explicitly from one BidSubmission and exact Ready scope version. Draft items can be edited; Ready for Comparison freezes summary, independent commercial items, coverage and evidence. Base Bid/currency/tax and alternates/allowances/fees/exclusions/conditions stay separate; missing is never zero or inferred excluded. Provider proposals are explicit OpenAI-only, native-text PDF-only (bounded size/text), suggestion-only and source-excerpt-validated; scanned PDFs and Office/spreadsheet formats retain full manual structuring. No extraction request occurs during upload, webhook or page load. Project 3 validation preserves three submissions/attachments, one Ready revision and four extraction runs; qualification remains Not reviewed. M3-08 comparison and recommendation are not started.
