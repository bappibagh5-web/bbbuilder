...
Current update (2026-09-17): M3-01 through M3-08 are COMPLETE and manually validated; M3-09 is NEXT / NOT STARTED. M3-08 requires explicit human selection of Ready BidRevisions for one exact frozen ScopePackageVersion, keeps submitted bids immutable, preserves Not addressed separately from Excluded, and uses explicit BB Builders Decimal ADD/DEDUCT adjustments without FX, automatic commercial normalization, ranking, recommendation or award. Project 3 preserves one Ready HVAC comparison on scope version 79 with the priced ACR and D.Peppard revisions selected and one explicit ACR permit/fee leveling adjustment; the historical no-price D.Peppard revision remains unselected. M3-07 preserves immutable quote intake and adds human-controlled structured BidRevisions plus explicit, suggestion-only native-text PDF extraction. Schema V3 live validation passed with `gpt-5-mini`. Check Git and `docs/CURRENT_STATUS.md` for authoritative state; older checkpoints below are historical.

M3-07 commercial truth is human-reviewed BidRevision data layered over immutable quote files, never AI output. A structured revision is created explicitly from one BidSubmission and exact Ready scope version. Draft items can be edited; Ready for Comparison freezes summary, independent commercial items, coverage and evidence. Base Bid/currency/tax and alternates/allowances/fees/exclusions/conditions stay separate; missing is never zero or inferred excluded. Provider proposals are explicit OpenAI-only, native-text PDF-only (bounded size/text), suggestion-only and source-excerpt-validated; scanned PDFs and Office/spreadsheet formats retain full manual structuring. No extraction request occurs during upload, webhook or page load. M3-08 comparison and leveling are complete; recommendation, winner selection and award remain outside M3-08. M3-09 is NEXT / NOT STARTED.
