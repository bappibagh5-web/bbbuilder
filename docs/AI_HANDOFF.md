...
Current update (2026-09-21): Milestones 1 through 4 plus PRE-M5-01 through PRE-M5-04 are COMPLETE and manually validated. PRE-M5-05 is IMPLEMENTED / MANUAL VALIDATION PENDING; M5 and Prospecting remain NOT STARTED. The production sidebar contains only supported production destinations. Bid Opportunities is hidden rather than represented as a fake feature, while its isolated fixture code remains preserved outside production navigation. Global demo metadata and non-functional shell search/notification controls are removed. Dashboard, Projects, Subcontractors, Campaigns, Comparisons, Proposals, Awarded Projects, Activity, and Settings use established production read models and permissions. Settings uses existing Organization/Membership truth, preserves history, blocks self-lockout/last-Admin failures, and keeps encrypted email/webhook configuration credential-safe. No Project 3 data or provider was touched. Check Git and `docs/CURRENT_STATUS.md` for authoritative state; older checkpoints below are historical.
