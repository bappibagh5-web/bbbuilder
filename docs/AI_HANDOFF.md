...
Before M3-01, the Dashboard received the same summary-first performance treatment and awaits final manual browser acceptance. Its former 31-request local profiling path took 68.60 seconds, issued 5,971 queries, and transferred approximately 3.43 MB because it downloaded per-project documents/conflicts/snapshots/activity and per-document runs/findings. The initial performance-only `/api/v1/organizations/{slug}/dashboard/` response averaged 76 ms, 7 queries, and 697 bytes across ten runs; `/dashboard/activity/` independently averaged 10 ms, 3 queries, and 2.17 KB for eight safe events. A subsequent semantic correction selects the authoritative latest successful project-set review where present, confirms frozen selected revisions are still current, and falls back to standalone document reviews only for projects without project-set analysis. The corrected summary averaged 99 ms, 9 queries, and 800 bytes across ten HTTP requests. Thus Project 3 reports Run 18's 556 findings, 509 AI handled, 47 human reviewed, 0 attention/conflicts, 23 selected documents / 141 pages, and approved Project Information V4; unrelated historical document runs do not inflate the Dashboard. Refresh keeps existing content visible. M3-01 remains not started.
