...
Current update (2026-09-14): M3-01 outreach domain foundation is committed. M3-02 deterministic RFQ Template V3, M3-03 explicit approval/recipient preparation, and M3-04 guarded SMTP delivery are COMPLETE and manually validated. The `apps.outreach` campaign keeps its exact Ready scope version; RFQ Template V3 uses only that scope and stored project facts. M3-04 adds backend send blockers, a separate human batch-send approval, immutable personalized messages, and durable attempt history. Real Resend SMTP connection testing succeeded and one controlled test email was received; no contractor invitation was sent. Project 3 campaign #1 remains blocked by missing Bid Due, no message/attempt/approval exists, and D. Peppard has not been emailed. M3-05 is NEXT / NOT STARTED. Check Git and `docs/CURRENT_STATUS.md` for authoritative state; older checkpoints below are historical.

M3-04 live-email follow-up: replaces the fake delivery adapter with organization-scoped, Admin-managed SMTP settings and Fernet-encrypted password storage (disabled by default). A private, stable `OUTREACH_CREDENTIAL_ENCRYPTION_KEY` must be set on every backend host; without it, saving/using credentials fails closed. Sender identity and human campaign deadlines freeze into immutable message versions and stale prior approvals when changed. Settings has explicit connection-only and controlled test-email actions with visible safe results and send-test blockers. Tests mock SMTP; real Resend connection and controlled-mailbox delivery were manually validated. Connection-test history is diagnostic, not a permanent readiness gate. Do not send to D. Peppard. No M3-05 work is included.
