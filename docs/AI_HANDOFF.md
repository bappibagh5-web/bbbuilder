...
Current update (2026-09-19): M3-01 through M3-10 and Milestone 3 are COMPLETE and manually validated. M4-01 is COMPLETE / MANUALLY VALIDATED; M4-02 is NEXT / NOT STARTED. M4-01 introduces separate stable Estimate and Proposal identities with append-only Draft versions; every ProposalVersion freezes one exact EstimateVersion, optional project contact, actor, status, and creation time. Project 3 validates Estimate V1/V2 and Proposal V1→Estimate V1 / Proposal V2→Estimate V2 across hard refresh, with Viewer read-only. Creation remained explicit: M3 Selected for Proposal did not create a proposal, and M3 subcontractor pricing was not copied as client sell pricing. There are no financial calculations, selected-bid assembly, PDFs, finalization, awards, subcontracts, or purchase orders. Project 3 still preserves the finalized V1 HVAC review on comparison #1/scope version 79 and all source M3 evidence. Check Git and `docs/CURRENT_STATUS.md` for authoritative state; older checkpoints below are historical.
