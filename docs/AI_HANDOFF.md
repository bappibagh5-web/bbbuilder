...
Current update (2026-09-17): M3-01 through M3-09 are COMPLETE and manually validated; M3-10 is NEXT / NOT STARTED. M3-09 adds an explicit, versioned human review over an exact Ready M3-08 comparison: every bid begins Undecided, procurement shortlisting is human-only and separate from discovery shortlisting, lowest price never drives selection, and the overall outcome is Selected for Proposal or No Acceptable Bid with required rationale. Finalized reviews are immutable and corrections require a successor. Selected for Proposal is not an award. Project 3 preserves one finalized V1 HVAC review on comparison #1/scope version 79: ACR Ready BidRevision #4 is explicitly Shortlisted and Selected for Proposal, D.Peppard Ready BidRevision #5 is explicitly Not Shortlisted, and the exact frozen comparison, source bids, CAD 3,000 ACR adjustment, discovery shortlist and outreach state remain unchanged. Check Git and `docs/CURRENT_STATUS.md` for authoritative state; older checkpoints below are historical.

M3-07 commercial truth is human-reviewed BidRevision data layered over immutable quote files, never AI output. A structured revision is created explicitly from one BidSubmission and exact Ready scope version. Draft items can be edited; Ready for Comparison freezes summary, independent commercial items, coverage and evidence. Base Bid/currency/tax and alternates/allowances/fees/exclusions/conditions stay separate; missing is never zero or inferred excluded. Provider proposals are explicit OpenAI-only, native-text PDF-only (bounded size/text), suggestion-only and source-excerpt-validated; scanned PDFs and Office/spreadsheet formats retain full manual structuring. No extraction request occurs during upload, webhook or page load. M3-08 comparison/leveling and M3-09 explicit human selection are complete; award remains outside both. M3-10 is NEXT / NOT STARTED.
