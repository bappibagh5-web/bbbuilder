...
Current update (2026-09-17): M3-01 through M3-10 and Milestone 3 are COMPLETE and manually validated; M4 is NEXT / NOT STARTED. Project 3 preserves one finalized V1 HVAC review on comparison #1/scope version 79: ACR Ready BidRevision #4 is explicitly Shortlisted and Selected for Proposal, D.Peppard Ready BidRevision #5 is explicitly Not Shortlisted, and the exact frozen comparison, source bids, CAD 3,000 ACR adjustment, discovery shortlist and outreach state remain unchanged. One explicit ACR qualification decision is persisted as Qualified and Viewer read-only access was manually validated across Outreach, Ready bids/evidence, comparison, and finalized review. Milestone 3 used controlled validation quote PDFs for commercial comparison; it did not claim genuine contractor-issued pricing. Real SMTP, Resend webhook/Receiving, response, attachment intake, private download, structured-bid, comparison, and human-decision infrastructure was validated. Selected for Proposal is not an award, and no proposal/award domain exists yet. Check Git and `docs/CURRENT_STATUS.md` for authoritative state; older checkpoints below are historical.
