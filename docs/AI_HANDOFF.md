...
Current update (2026-09-20): Milestones 1 through 4 plus PRE-M5-01, PRE-M5-02, and PRE-M5-03 are COMPLETE and manually validated. PRE-M5-04 is NEXT / NOT STARTED, and M5 remains NOT STARTED. Global Activity now reads bounded, paginated organization-scoped `AuditEvent` summaries with controlled labels, safe project links, and project/activity-family filters; it never returns raw audit metadata or demo rows. Global Campaigns, Comparisons, Proposals, and Subcontractors are bounded organization-scoped directories over existing production truth and link back to exact project workspaces. Campaign summaries omit full communications and separate setup lifecycle from actual delivery counts, comparison summaries never infer rank/winner/recommendation, and proposal summaries expose client-facing totals and explicit issue dates only while excluding internal procurement costs and rates. The global Subcontractors route uses the existing Company/Contact/TradeCapability and project procurement history, with lazy detail. Project participation relates to shared Company truth; same-project/same-trade historical candidate records may be grouped visually without deletion. Qualification remains invitation-specific history rather than a global contractor label, no duplicate business domain was introduced, and Prospecting remains separate/unimplemented. The award boundary requires separate explicit human Project Award and Trade Award decisions, preserves exact Finalized Proposal/Frozen Estimate and M3 selected-bid sources, and creates an immutable versioned M5-ready handoff only after an explicit Confirmed Project Award transition. It does not create subcontracts, purchase orders, notifications or external sync. Project 3 contains one controlled confirmed Project Award, one controlled confirmed ACR trade award and handoff snapshot V1 and is explicitly Awarded; these are validation records only and do not represent a real JD Sports commercial award. Check Git and `docs/CURRENT_STATUS.md` for authoritative state; older checkpoints below are historical.

M3-07 commercial truth is human-reviewed BidRevision data layered over immutable quote files, never AI output. A structured revision is created explicitly from one BidSubmission and exact Ready scope version. Draft items can be edited; Ready for Comparison freezes summary, independent commercial items, coverage and evidence. Base Bid/currency/tax and alternates/allowances/fees/exclusions/conditions stay separate; missing is never zero or inferred excluded. Provider proposals are explicit OpenAI-only, native-text PDF-only (bounded size/text), suggestion-only and source-excerpt-validated; scanned PDFs and Office/spreadsheet formats retain full manual structuring. No extraction request occurs during upload, webhook or page load. M3-08 comparison/leveling, M3-09 explicit human selection, and M3-10 integrated acceptance are complete; proposal and award remain M4 work.

M3-06 is an explicit human intake boundary: manual uploads or provider attachments from an already exact-correlated inbound response become immutable QuoteSubmission/BidAttachment/FileAsset history bound to the exact invitation and Ready scope version. The Bids tab is production-backed and Viewer-readable with private authenticated file downloads. A received webhook alone remains metadata-only. No Project 3 quote is imported automatically; Resend signed attachment URLs are fetched only during an authorized import action. No pricing extraction, leveling, award, provider send, or M3-07 work is included.

M3-04 live-email follow-up: replaces the fake delivery adapter with organization-scoped, Admin-managed SMTP settings and Fernet-encrypted password storage (disabled by default). A private, stable `OUTREACH_CREDENTIAL_ENCRYPTION_KEY` must be set on every backend host; without it, saving/using credentials fails closed. Sender identity and human campaign deadlines freeze into immutable message versions and stale prior approvals when changed. Settings has explicit connection-only and controlled test-email actions with visible safe results and send-test blockers. Tests mock SMTP; real Resend connection and controlled-mailbox delivery were manually validated. Connection-test history is diagnostic, not a permanent readiness gate. Do not send to D. Peppard.

M3-05 receives only Svix-signed Resend webhooks (five-minute timestamp tolerance, durable event-ID dedupe) at an organization-specific endpoint whose signing secret is Fernet-encrypted in the database. Admin must configure a publicly reachable HTTPS webhook in the Resend dashboard and save its `whsec_` secret in Settings. Subscribe to sent, delivered, delivery delayed, bounced, failed, complained, opened, clicked and received events. Resend/SES may replace the submitted RFC Message-ID with a provider-generated one. Outbound correlation accepts the exact persisted submitted ID when preserved, a previously verified provider email ID, or a unique full-content match from Resend's sent-email API. If multiple older messages have identical content, only a unique bounded provider-creation/delivery-attempt window disambiguates; otherwise the event remains unassigned. Subject/address/timing alone never assign an event. Inbound webhooks omit text and reply headers: backend fetches the specific received email using the encrypted SMTP credential and an explicit application User-Agent, then requires exact `In-Reply-To`/`References`, organization and address checks. An unassigned stored reply may be enriched and assigned in place on signed replay, without a duplicate response. Incoming attachments are metadata only until M3-06. Provider-reported opens/clicks never qualify or respond for a human.
...
- Milestone 2: complete, including accepted Scopes/Contractors lazy-loading performance polish; this checkpoint is historical and predates M3-01
- Working tree: clean
- Ahead/behind: `0/0`
...
Milestones 1 and 2 are complete. M2A-01, M2B-01 through M2B-06, and the final summary-first Scopes/Contractors UI were manually accepted. Project 3 Project Information V4 retains 27 current trade packages and 659 detailed ScopeItems, with 106 project-wide requirements separate. Scopes returns a 4-query, approximately 14.1 KB summary (five-run local average 0.96 seconds versus approximately 19.46 seconds, at least 8,997 queries, and 2.36 MB before), starts all trade rows collapsed, and lazily caches exact package details. Contractor Discovery initially loads only its 5-query coverage response (16 ms average, 738 bytes), then lazily caches candidates per expanded Ready trade; HVAC's 31 active production-facing candidates averaged 29 ms and 23.2 KB. Profiles, enrichment, searches, mutations, and history remain user-triggered. M3-01 through M3-05 are complete and manually validated. No provider search or outreach send is part of page load. Project 3's controlled test-mailbox response is preserved; M3-06 is next and not started.

Before M3-01, the Dashboard received summary-first performance treatment. Its corrected summary averaged 99 ms, 9 queries, and 800 bytes across ten HTTP requests. Project 3 reports Run 18's 556 findings, 509 AI handled, 47 human reviewed, 0 attention/conflicts, 23 selected documents / 141 pages, and approved Project Information V4; unrelated historical document runs do not inflate the Dashboard. Refresh keeps existing content visible. This paragraph records a historical pre-M3-01 checkpoint; M3-01 is now implemented awaiting manual validation.
