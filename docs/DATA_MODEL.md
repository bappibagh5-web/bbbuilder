...
`BidExtractionRun` is a separate durable, explicitly requested OpenAI proposal for a bounded native-text PDF attachment. It stores safe provider/model/schema version, usage/request ID, grounded candidate JSON and terminal status. `BidCandidateDecision` records explicit human accept/correct/ignore actions for a Draft revision; extraction never creates Ready commercial truth. Office/spreadsheet and image-only quote files remain manually structurable; this task does not create another Document source-of-truth. M3-08 cross-contractor comparison/leveling and later selection models are not implemented.
