...
| Estimate / EstimateVersion | Implemented in M4-01 as one stable project estimate identity with append-only Draft versions; financial lines/calculations begin later. |
| Proposal | Implemented in M4-01 as the stable client-proposal identity for a project and its Estimate. |
| ProposalVersion | Implemented in M4-01 as an append-only Draft revision bound to one exact EstimateVersion; commercial content/finalization begin later. |
| ProposalPdfArtifact | M4-06 append-only, versioned and template-versioned private PDF artifact bound to an exact Finalized ProposalVersion and one immutable FileAsset. |
...
M3-04 adds `BatchSendApproval` (append-only authorized human approvals bound to a batch's exact prepared-message fingerprint), `OutreachDeliveryAttempt` (per-message sequence, stable message idempotency key, provider key, pending/succeeded/failed, timestamps and safe outcome), and a controlled `invited` recipient status/event only after successful delivery. Existing `OutreachMessage` remains an immutable personalized content snapshot. Changed content requires a new message version and a new approval; earlier versions and approvals remain intact. A completed attempt is immutable; a new retry is a new row. No opened/responded/bid entities are included.

M3-04 live SMTP support adds campaign Bid Due/Questions Due and a monotonic setup version, append-only `CampaignSetupEvent`, one Admin-managed `OutreachSenderSettings` per organization, and immutable OutreachMessage snapshots of template/scope/setup/deadlines. `BatchSendApproval` also freezes setup version. `OutreachDeliveryAttempt` adds an outcome-uncertain state that blocks resend. `OutreachSMTPConfiguration` stores each organization's nonsecret connection settings and Fernet-encrypted password ciphertext; the decryption key remains environment-only. No API response or audit event includes the ciphertext or decrypted secret.

M3-05 adds `ResendWebhookConfiguration` (organization, opaque endpoint token, Fernet-encrypted signing secret, enabled state), immutable `ResendWebhookEvent` (unique signed event ID, type, timestamps and safe provider IDs), and immutable `OutreachProviderEmail` (verified provider email ID + RFC Message-ID bound to one exact sent message). `OutreachResponse` stores inbound/manual response evidence and may be deliberately unassigned when no unique reliable thread/contact match exists; inbound attachments are count-only until explicit M3-06 intake. `OutreachQualificationDecision` is append-only human judgment with separate current qualification state on `InvitationRecipient`. Recipient delivery/engagement/response axes and append-only `InvitationRecipientStatusEvent` preserve earlier states without treating opens/clicks as responses.

M3-06 adds immutable `BidSubmission` (organization/project, exact campaign/batch/recipient/company/contact and frozen ScopePackageVersion, manual/inbound source, optional unique correlated OutreachResponse, received time and human recorder) and immutable `BidAttachment` (submission, one immutable FileAsset, original name, checked MIME/size/SHA-256, optional provider attachment ID). A request key or correlated response prevents repeated intake from duplicating history. The recipient's Bid Submitted status is projected through its append-only status-event history; human qualification remains separate. Original quote files stay in private object storage and are never represented by public provider download URLs.

M3-07 adds `BidRevision` as an explicitly created, human-owned commercial interpretation of one BidSubmission; it freezes organization, project, exact ScopePackage/Ready ScopePackageVersion, company and InvitationRecipient. Sequence and optional `supersedes` preserve contractor revision history without automatically treating later submissions as replacements. Draft summary fields include Decimal Base Bid, explicit currency review, tax treatment, stated validity/schedule and estimator notes. `BidCommercialItem` keeps alternates/options, allowances, permits/fees/taxes, explicit exclusions and conditions separate from Base Bid; amount is exact Decimal and treatment/included-in-base are explicit. `BidScopeCoverage` references only ScopeItems from that exact quoted Ready version, distinguishing confirmed included/excluded/qualified from not addressed/needs clarification. `BidEvidence` ties a summary field, item or coverage decision to the exact BidAttachment and optional 1-based PDF page/exact excerpt, or a human note. Ready for Comparison freezes the summary and child commercial meaning; corrections require a successor revision, not a rewrite.

`BidExtractionRun` is a separate durable, explicitly requested OpenAI proposal for a bounded native-text PDF attachment. It stores safe provider/model/schema version, usage/request ID, grounded candidate JSON and terminal status. Schema V3 carries explicit allowance inclusion and distinguishes procurement/equipment lead time from work schedule. Human acceptance projects validity and work schedule into dedicated BidRevision summary fields and confirms grounded Base Bid currency; it does not invent an exact ScopeItem association. `BidCandidateDecision` records explicit human accept/correct/ignore actions for a Draft revision; extraction never creates Ready commercial truth. Office/spreadsheet and image-only quote files remain manually structurable; this task does not create another Document source-of-truth.

M3-08 adds `BidComparison`, `BidComparisonEntry`, and `BidLevelingAdjustment`. A comparison freezes organization, project, ScopePackage and exact Ready ScopePackageVersion, with sequence and optional supersession preserving later correction history. Each entry snapshots bidder identity while referencing one explicitly selected immutable Ready BidRevision; one company may appear only once per comparison. Adjustments are BB Builders comparison data, not contractor quote revisions: each records ADD/DEDUCT direction, exact Decimal amount, currency, controlled category, rationale, human actor/time, and optional exact ScopeItem or source BidCommercialItem. Ready for Human Review freezes entries and adjustments. The read model joins immutable commercial terms, evidence and exact-version scope coverage without converting missing coverage into exclusion or unlinked statements into guessed scope.

M3-09 adds `BidHumanReview` and `BidHumanDecision`. A review is organization/project scoped and permanently bound to one exact Ready BidComparison, ScopePackage and ScopePackageVersion. Sequence plus optional `supersedes` preserves append-only correction history. One decision row exists for each frozen BidComparisonEntry and stores explicit Undecided, Shortlisted or Not Shortlisted state, human note, actor and time. The review stores Draft/Finalized lifecycle, Selected for Proposal or No Acceptable Bid outcome, optional exact selected comparison entry, required rationale and finalizer/time. A finalized review and its decisions are immutable; source comparison and quote records are never rewritten.

M4-01 adds `Estimate`, `EstimateVersion`, `Proposal`, and `ProposalVersion`. Estimate and Proposal are stable one-per-project identities with protected organization/project/creator relationships. EstimateVersion and ProposalVersion use positive project-local sequences and optional one-to-one predecessor links so correction is append-only. Each Proposal references its Estimate and may reference one existing ProjectContact; each ProposalVersion freezes one exact EstimateVersion and never dynamically follows the latest estimate. All M4-01 versions are Draft and creation is explicit. Project 3 V1/V2 history and exact cross-version bindings were manually validated. No financial line, total, selected-bid snapshot, rendered document, finalization, award, subcontract, or purchase-order entity is included.

M4-02/M4-03/M4-04 add `EstimateLine`, `EstimateAllowance`, `EstimateAlternate`, `EstimateExclusion`, and `EstimateFinancialAdjustment` under one exact EstimateVersion. EstimateLine distinguishes a selected contractor Base Bid from each M3 BB Builders leveling adjustment and protects exact BidHumanReview, BidComparisonEntry, BidRevision, ScopePackageVersion, Company, and optional BidLevelingAdjustment references while also freezing display labels. Allowances retain included/excluded treatment and nullable amount; alternates retain ADD/DEDUCT, nullable amount, and explicit estimate inclusion; exclusions are non-arithmetic statements. Financial adjustments retain controlled category, fixed/percentage method, explicit basis/rate or amount, currency, deterministic calculated amount, sequence, and actors. All money is Decimal. Historical versions retain their own lines and never query later M3 state for financial meaning. ProposalVersion continues to bind the exact EstimateVersion whose calculation is displayed.

M4-05/M4-06/M4-07 extend EstimateVersion with Draft/Frozen lifecycle and freeze actor/time/calculation fingerprint. ProposalVersion adds Draft/Finalized lifecycle, controlled client-facing text, issue/contact metadata, structured immutable client/project and commercial snapshots, proposal identity, snapshot fingerprint and finalizer/time. Finalization freezes both exact versions atomically. `ProposalPdfArtifact` protects one immutable private FileAsset plus artifact sequence, generation actor/time and template version; multiple append-only artifacts may preserve corrected renderings for the same finalized ProposalVersion without overwriting history. Generation for one template version is idempotent, and every PDF derives only from the finalized snapshot rather than mutable upstream records.

M4-08/M4-09/M4-10 add `ProjectAward`, `TradeAward`, and `AwardedProjectHandoffSnapshot`. ProjectAward freezes one exact Finalized ProposalVersion and its exact Frozen EstimateVersion plus explicit client-accepted amount/currency/date/rationale/reference/evidence. TradeAward freezes one exact finalized M3 human review, comparison entry, Ready BidRevision, ScopePackageVersion and Company; quoted Base Bid, internal evaluated amount, and explicit awarded amount remain separate. Both decisions use Draft/Confirmed lifecycle, actors/timestamps, sequence and predecessor links so confirmed history is immutable and correction appends a successor. A confirmed ProjectAward is the sole source for an explicit Project status transition to Awarded. The handoff snapshot is immutable, fingerprinted and versioned for later M5 consumption; it performs no external sync.

M4-08 through M4-10 and Milestone 4 are complete and manually validated. Project 3 award rows are controlled validation records, not evidence of a real JD Sports commercial award. M5 remains not started.
