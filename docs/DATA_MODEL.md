...
The uncommitted live SMTP follow-up adds campaign Bid Due/Questions Due and a monotonic setup version, append-only `CampaignSetupEvent`, one Admin-managed `OutreachSenderSettings` per organization, and immutable OutreachMessage snapshots of template/scope/setup/deadlines. `BatchSendApproval` also freezes setup version. `OutreachDeliveryAttempt` adds an outcome-uncertain state that blocks resend. `OutreachSMTPConfiguration` stores each organization's nonsecret connection settings and Fernet-encrypted password ciphertext; the decryption key remains environment-only. No API response or audit event includes the ciphertext or decrypted secret.
