...
M3-08 adds `BidComparison`, `BidComparisonEntry`, and `BidLevelingAdjustment`. A comparison freezes organization, project, ScopePackage and exact Ready ScopePackageVersion, with sequence and optional supersession preserving later correction history. Each entry snapshots bidder identity while referencing one explicitly selected immutable Ready BidRevision; one company may appear only once per comparison. Adjustments are BB Builders comparison data, not contractor quote revisions: each records ADD/DEDUCT direction, exact Decimal amount, currency, controlled category, rationale, human actor/time, and optional exact ScopeItem or source BidCommercialItem. Ready for Human Review freezes entries and adjustments. The read model joins immutable commercial terms, evidence and exact-version scope coverage without converting missing coverage into exclusion or unlinked statements into guessed scope. M3-09 contractor selection/recommendation models are not implemented.
