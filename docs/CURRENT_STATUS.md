...
The post-Milestone-2 Dashboard performance fix is implemented pending final manual acceptance. The browser no longer assembles dashboard totals by downloading every active project's documents, conflicts, snapshot manifests, audit history, analysis runs, findings, and provenance. A read-only organization-scoped summary endpoint returns only KPI/project-card data; recent activity is independent and capped at eight safe events. Against the same Project 3 data, the previous 31-request profiling harness took 68.60 seconds, issued 5,971 database queries, and transferred approximately 3.43 MB. The initial performance-only summary averaged 76 ms, 7 queries, and 697 bytes across ten local runs; activity averaged 10 ms, 3 queries, and 2.17 KB. Final semantic correction makes the latest successful project-set review authoritative (legacy document runs are fallback only), so JD Sports Run 18 reports its 23 selected documents / 141 pages, 556 findings, 509 AI handled, 47 human reviewed, no attention/conflicts, and approved Project Information V4 without misleading 2-of-24 document coverage. The corrected HTTP summary averaged 99 ms, 9 queries, and 800 bytes across ten local runs. The dashboard shell renders immediately, refresh preserves the last successful data, and activity failure cannot blank project summaries. M3-01 remains next and has not started.
...
M1-04 through M1-12 are complete. Milestone 1 is complete; M2A-01 and M2B-01 through M2B-06 are complete, and outreach/M3 has not started.
