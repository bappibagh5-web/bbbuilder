...
Schema V2 live validation passed with `gpt-5-mini` and retained exact page/excerpt provenance while normalizing explicit permit language to fee/included. Historical provider-unavailable Runs 2 and 3 were caused by restricted local Celery outbound socket access; authenticated model access was separately proven. Production workers that execute provider tasks must therefore run with normal outbound HTTPS access. Provider failure history remains append-only and is never rewritten after a later successful validation.
