  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_19rem]">
      <main className="min-w-0 space-y-5">{selectedRun?.status === "succeeded" && <HumanReviewPanel run={selectedRun} findings={findings} conflicts={conflicts} canOperate={canOperate} actions={reviewActions} slug={slug} projectId={project.id} acting={acting} setActing={setActing} setError={setError} refresh={refreshReview} onViewSource={(source) => void viewDocument(sourceDocumentTarget(selectedRun.document, source.document_revision, source.page_number))} />}
        <ProjectIntelligencePanel slug={slug} projectId={project.id} canOperate={canOperate} revisionOptions={options} onSnapshotsChange={setProjectSnapshots} authoritativeRunId={projectReview?.current_run?.id} />
      </main>
...
function ProjectIntelligencePanel({ slug, projectId, canOperate, onSnapshotsChange, authoritativeRunId }: { slug: string; projectId: number; canOperate: boolean; revisionOptions: RevisionOption[]; onSnapshotsChange: (snapshots: IntelligenceSnapshot[]) => void; authoritativeRunId?: number }) {
  ...
  const loadCandidates = useCallback(async () => {
    ...
    try {
      const payload = await analysisApi.intelligenceCandidates(slug, projectId);
      setCandidates([...payload.candidate_runs].sort((left, right) => {
        if (left.id === authoritativeRunId) return -1;
        if (right.id === authoritativeRunId) return 1;
        return right.id - left.id;
      }));
    } finally {
      setCandidatesLoading(false);
    }
  }, [authoritativeRunId, projectId, slug]);
  const loadSnapshots = useCallback(async () => {
    setSnapshotsLoading(true);
    try {
      const history = await analysisApi.snapshots(slug, projectId);
  ...
  const load = useCallback(async () => {
    const results = await Promise.allSettled([loadCandidates(), loadSnapshots()]);
    const failure = results.find((result) => result.status === "rejected");
    if (failure?.status === "rejected") throw failure.reason;
  }, [loadCandidates, loadSnapshots]);
  useEffect(() => {
    const timer = window.setTimeout(() => {
  ...
  if (candidatesLoading || snapshotsLoading) {
    return <section className="space-y-4 border-t-4 border-slate-200 pt-5"><Card className="p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-semibold uppercase tracking-wider text-blue-700">{documentReviewScopeCopy.projectWideHeading}</p><span className="rounded-full bg-violet-50 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-violet-700">{documentReviewScopeCopy.projectWideBadge}</span></div><h3 className="mt-2 font-semibold">Project information for approval</h3><p className="mt-1 max-w-2xl text-sm text-slate-600">{documentReviewScopeCopy.projectWideExplanation}</p></div><button type="button" disabled className="h-10 rounded-lg border px-3 text-sm font-semibold opacity-50">Refresh</button></div><div className="mt-5"><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Current project-wide review</p>{candidatesLoading ? <p className="mt-2 text-sm text-slate-500">Loading current project review…</p> : currentProjectReview ? <p className="mt-2 text-sm font-semibold text-slate-800">Project-wide review · {currentProjectReview.document_count} documents · {currentProjectReview.page_count} pages/slides</p> : <p className="mt-2 text-sm text-slate-500">No completed project-wide review is ready yet.</p>}</div><div className="mt-5 rounded-xl border bg-slate-50 p-4"><p className="text-sm font-semibold text-slate-700">Project information history</p>{snapshotsLoading ? <p className="mt-2 text-sm text-slate-500">Loading project information versions…</p> : <p className="mt-2 text-sm text-slate-600">{snapshots.length} project information version{snapshots.length === 1 ? "" : "s"}</p>}</div></Card></section>;
  }
