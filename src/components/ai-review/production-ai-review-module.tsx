...
import { categoryLabel, decisionLabel, documentReviewScopeCopy, documentVersionLabel, plainAnalysisStatus, progressPresentation, reviewCounts, reviewStages, snapshotPresentation, versionDetailsButtonLabel, versionDetailsInitiallyExpanded, versionSummary } from "@/lib/document-review-presentation";
...
export function ProductionAIReviewModule({ project, membership }: { project: ProductionProject; membership: OrganizationMembership }) {
  ...
  if (!options.length) return <ModuleState icon={FileSearch} title="No documents ready for review" detail="Upload a document first. We’ll check it, organize its pages, and show when it is ready for review." />;
  ...
  async function viewDocument() {
    ...
    setError(null);
    try {
      const blob = await documentsApi.download(slug, project.id, selected.document.id, selected.revision.id);
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_19rem]">
      <main className="min-w-0 space-y-5">{selectedRun?.status === "succeeded" && <HumanReviewPanel run={selectedRun} findings={findings} conflicts={conflicts} canOperate={canOperate} actions={reviewActions} slug={slug} projectId={project.id} acting={acting} setActing={setActing} setError={setError} refresh={refreshReview} onViewSource={() => void viewDocument()} />}
