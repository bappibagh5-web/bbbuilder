...
export function ProductionContractorsModule({ project, membership }: { project: ProductionProject; membership: OrganizationMembership }) {
  ...
  const [profileCandidate, setProfileCandidate] = useState<ContractorCandidate | null>(null);
  ...
  async function setApproval(candidate: ContractorCandidate, approved: boolean) {
    setBusy(candidate.scope_package);
    setError(null);
    try {
      const updated = await contractorsApi.setStatus(slug, project.id, candidate.id, approved ? "approved_for_outreach" : "shortlisted");
      setCandidates((current) => ({ ...current, [candidate.scope_package]: replaceCandidate(current[candidate.scope_package] ?? [], updated) }));
      setProfileCandidate(updated);
      setCoverage((await contractorsApi.coverage(slug, project.id)).trades);
      if (profile?.id === candidate.company.id) setProfile(await contractorsApi.profile(slug, project.id, profile.id));
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Outreach approval could not be changed.");
  ...
  async function saveContact(contactId: number | null, input: ContractorContactInput) {
    ...
    try {
      ...
      if (profileCandidate) {
        const rows = await contractorsApi.candidates(slug, project.id, undefined, profileCandidate.scope_package);
        setCandidates((current) => ({ ...current, [profileCandidate.scope_package]: rows }));
        setProfileCandidate(rows.find((row) => row.id === profileCandidate.id) ?? null);
      }
    } catch (reason) {
  ...
  return <div className="space-y-5">
    ...
    {loading ? <ContractorSummarySkeleton /> : ready.length === 0 ? <Empty title="No Ready scope packages" detail="Mark a scope Ready before searching for contractors." /> : <>
      ...
      {profile && <div ref={profileRef}><ContractorProfilePanel key={profile.id} profile={profile} outreachCandidate={profileCandidate} canManage={canManage} busy={profileBusy || busy !== null} onBack={() => { setProfile(null); setProfileCandidate(null); }} onSave={saveContact} onFindContactDetails={findContactDetails} onApprovalChange={(approved) => { if (profileCandidate) void setApproval(profileCandidate, approved); }} /></div>}
      {ready.map((scope) => <ScopeDiscovery key={scope.scope_package} scope={scope} rows={filterContractors(candidates[scope.scope_package] ?? [], shortlistedOnly)} shortlistedOnly={shortlistedOnly} city={project.city} province={project.province_state} minimumTarget={minimumTarget} canManage={canManage} busy={busy === scope.scope_package || profileBusy} expanded={expanded === scope.scope_package} loading={candidateLoading === scope.scope_package} error={candidateErrors[scope.scope_package]} searchMessage={searchMessages[scope.scope_package]} onToggle={() => void toggleTrade(scope.scope_package)} onSearch={search} onSetShortlist={setShortlist} onApprovalChange={setApproval} onViewProfile={openProfile} />)}
    </>}
  </div>;
}
...
function CandidateRow({ candidate, rank, canManage, busy, onSetShortlist, onApprovalChange, onViewProfile }: { candidate: ContractorCandidate; rank: number; canManage: boolean; busy: boolean; onSetShortlist: (candidate: ContractorCandidate, shortlisted: boolean) => void; onApprovalChange: (candidate: ContractorCandidate, approved: boolean) => void; onViewProfile: (candidate: ContractorCandidate) => void }) {
  const shortlisted = candidate.status === "shortlisted";
  const approved = candidate.status === "approved_for_outreach";
  return <div className={`flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between ${shortlisted || approved ? "bg-emerald-50/60" : ""}`}>
    <div><div className="flex flex-wrap items-center gap-2"><span className="inline-flex h-6 min-w-6 items-center justify-center rounded-full bg-slate-900 px-1.5 text-xs font-bold text-white">{rank}</span><p className="font-semibold text-slate-900">{candidate.company.display_name}</p><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${candidate.company.source_type === "internal" ? "bg-blue-50 text-blue-700" : "bg-indigo-50 text-indigo-700"}`}>{contractorSourceLabel(candidate)}</span><span className="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-bold text-amber-700">Best Match · {candidate.match_score}/100</span><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-700">{contractorFitBand(candidate.match_score)}</span></div><p className="mt-1 text-sm text-slate-500">{candidate.company.address || `${candidate.company.city}, ${candidate.company.province}`}{candidate.company.phone ? ` · ${candidate.company.phone}` : ""}</p>{candidate.distance_miles !== null && <p className="mt-1 text-sm font-medium text-blue-700">{Math.round(candidate.distance_miles)} miles from project</p>}{candidate.google_rating !== null && <p className="mt-1 text-sm text-slate-600">Google {candidate.google_rating.toFixed(1)}{candidate.google_review_count !== null ? ` · ${candidate.google_review_count} reviews` : ""}</p>}<details className="mt-2"><summary className="cursor-pointer text-xs font-semibold text-blue-700">Why this ranks here</summary><ul className="mt-2 grid gap-1 text-xs text-slate-600 sm:grid-cols-2">{contractorRankingSignals(candidate).map((reason) => <li key={reason}>• {reason}</li>)}</ul></details>{candidate.company.website && <a href={candidate.company.website} target="_blank" rel="noreferrer" className="mt-2 inline-block text-sm font-medium text-blue-700 hover:underline">Visit website</a>}{(shortlisted || approved) && <p className="mt-2 text-xs font-semibold text-emerald-700">Shortlisted ✓ · {candidate.outreach_contact_ready ? "Contact Ready" : "Primary email contact required"}</p>}</div>
    <div className="flex flex-wrap gap-2">
      <button type="button" onClick={() => onViewProfile(candidate)} disabled={busy} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 disabled:opacity-50">View profile</button>
      {candidate.status === "candidate" && canManage && <button type="button" onClick={() => onSetShortlist(candidate, true)} disabled={busy} className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-700 disabled:opacity-50">Add to Shortlist</button>}
      {shortlisted && <>
        <span className="inline-flex items-center rounded-lg bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-700">Shortlisted ✓</span>
        {canManage && (candidate.outreach_eligibility.can_approve ? <button type="button" onClick={() => onApprovalChange(candidate, true)} disabled={busy} className="rounded-lg bg-[#173f5f] px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">Approve for Outreach</button> : <span className="self-center text-xs text-amber-700">{candidate.outreach_eligibility.reason}</span>)}
        {canManage && <button type="button" onClick={() => onSetShortlist(candidate, false)} disabled={busy} className="rounded-lg border px-3 py-2 text-sm font-semibold text-slate-700 disabled:opacity-50">Remove from shortlist</button>}
      </>}
      {approved && <>
        <span className="inline-flex items-center rounded-lg bg-indigo-50 px-3 py-2 text-sm font-semibold text-indigo-700">Approved for Outreach ✓</span>
        {canManage && (candidate.outreach_eligibility.can_revoke ? <button type="button" onClick={() => onApprovalChange(candidate, false)} disabled={busy} className="rounded-lg border px-3 py-2 text-sm font-semibold text-slate-700 disabled:opacity-50">Revoke approval</button> : <span className="self-center text-xs text-slate-500">{candidate.outreach_eligibility.reason}</span>)}
      </>}
      {candidate.status === "rejected" && <span className="text-sm text-slate-500">Rejected</span>}
    </div>
  </div>;
}
