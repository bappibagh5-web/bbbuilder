...
export function ProductionContractorsModule({ project, membership }: { project: ProductionProject; membership: OrganizationMembership }) {
  ...
  async function search(scope: ScopePackage, keywords: string) {
    ...
    try {
      ...
      setSearchMessages((current) => ({ ...current, [scope.id]: contractorSearchResultMessage(result.result_count) }));
    } catch (reason) {
      setSearchMessages((current) => ({ ...current, [scope.id]: "Contractor search could not be completed." }));
      setError(reason instanceof Error ? reason.message : "Contractor search could not be completed.");
    } finally {
      setBusy(null);
    }
  }
