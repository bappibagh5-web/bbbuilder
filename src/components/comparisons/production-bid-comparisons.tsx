...
function ComparisonWorkspace({ detail, choices, canEdit, busy, error, back, add, remove, saveNotes, adjust, removeAdjustment, ready, download }: { detail: ComparisonDetail; choices: ReadyBidChoice[]; canEdit: boolean; busy: boolean; error: string; back: () => void; add: (id: number) => void; remove: (id: number) => void; saveNotes: (notes: string) => void; adjust: (entryId: number, values: Record<string, unknown>) => void; removeAdjustment: (entryId: number, id: number) => void; ready: () => void; download: (entry: ComparisonDetail["entries"][number], attachment: { id: number; filename: string }) => void }) {
