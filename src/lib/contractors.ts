...
export const contractorsApi = {
  ...
  search(slug: string, projectId: number, input: { scope_package_id: number; city: string; province: string; country: string; radius_km?: number; keywords: string[] }) { return apiRequest<{ discovery_request_id: number; result_count: number; candidates: ContractorCandidate[] }>(`${base(slug, projectId)}/contractor-discovery/search/`, { method: "POST", body: JSON.stringify(input) }); },
